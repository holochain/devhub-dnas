...
function basic_tests () {
    ...
    it("should get wasm package", async function () {
	const wasm_pack			= await zomehub_csr.get_wasm_package( wasm1_addr );
	log.normal("%s", json.debug(wasm_pack) );

	expect( wasm_pack		).to.have.any.keys( "bytes" );
    });
